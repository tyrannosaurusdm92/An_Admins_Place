...
(function (global) {
  ...
  function renderEncounter(state, npc) {
    ...
    byId("conversationNpcMeta").textContent = `${npc.raceName || "User-defined ancestry"}${npc.lineageName ? ` · ${npc.lineageName}` : ""} · ${npc.profession || "community member"}`;
  ...
  function exportTranscript() {
    const state = LS.store.get(); const payload = { schema: "lifesimulation.dialogue-transcript.v1", exportedAt: LS.util.now(), project: { projectId: state.project.projectId, name: state.project.name }, conversations: state.conversations, players: state.dialoguePlayers, diagnostics: state.dialogueDiagnostics };
