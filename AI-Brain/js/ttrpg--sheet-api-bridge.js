...
(function(){
  ...
  function ready(){
    if(typeof window.collectSheet === 'function') {
      ...
      window.parent && window.parent.postMessage({type:'universal-sheet-ready'}, '*');
    }
  }
  window.addEventListener('message', ev=>{
    const msg = ev.data || {};
    if(msg.type === 'universal-apply-site-payload') applySitePayload(msg.payload);
    if(msg.type === 'universal-request-sheet-data' && window.UniversalSheetApi) ev.source && ev.source.postMessage({type:'universal-sheet-data', payload:window.UniversalSheetApi.collect()}, '*');
  });
