...
/* Universal Voice Studio - NPC/bot JSON import, plain-text scanning, and full JSON-pack matching */
(() => {
  'use strict';
  ...
  const inferFromPlainText = async (text = '') => {
    ...
    return {
      ...
      subclass,
      alignment,
      biome: window.UniversalBiomeAccents?.inferFromText(text) || '',
      universalAlignment: null,
  ...
  const normalizeNpcSync = (raw = {}) => {
    ...
    return {
      ...
      universalAlignment: pick('universalAlignment','alignmentAxes','axes') || null,
  ...
  window.UniversalVoiceJsonMapper = { normalizeNpc, normalizeNpcSync, inferFromPlainText, autoSelectForNpc, readJsonFile, makeNpcSummary };
})();
