...
/* Universal Voice Studio - Page controller */
(() => {
  'use strict';
  const Core = window.UniversalVoiceCore;
  const Analysis = window.UniversalVoiceAnalysis;
  const Synth = window.UniversalVoiceSynthesis;
  const Presets = window.UniversalVoicePresets;
  const Mapper = window.UniversalVoiceJsonMapper;
  const Exporter = window.UniversalVoiceExporter;
  const Emotions = window.UniversalVoiceEmotions;
  const Pack = window.UniversalVoiceGeneratorPack;
  const Accents = window.UniversalBiomeAccents;
  const Recorder = window.UniversalVoiceRecorder;
  const Humanization = () => window.UniversalVoiceHumanization;
  ...
  const readableEngine = () => {
    ...
    const vocalModel = window.UniversalVocalModel ? window.UniversalVocalModel.describeModel(state.profile, opts) : {};
    const plan = window.UniversalVoiceProsody ? window.UniversalVoiceProsody.previewPlan($('sampleText')?.value || 'this is what I sound like', state.profile, opts) : null;
    return {
      ...
      naturalnessGuard: window.UniversalNaturalnessGuard?.describe?.() || null,
      referenceAudioLearning: window.UNIVERSAL_REFERENCE_AUDIO_LEARNING?.overall || null,
  ...
  const collectNpcFromFields = () => ({
    ...
    universalAlignment: { Altruism:+$('axisAltruism').value, Lawfulness:+$('axisLawfulness').value, Cooperation:+$('axisCooperation').value, Honor:+$('axisHonor').value }
  });
  ...
  const exportBundle = () => ({
    ...
    dataPack: Pack ? { summary: Pack.packSummary(), counts: Pack.counts(), sourceFiles: Pack.sourceFiles(), humanizationResources: window.UNIVERSAL_VOICE_HUMANIZATION_REFERENCES?.resourceCounts || null } : null
  });
  ...
  const wireRecorder = () => {
    ...
    $('previewRecording')?.addEventListener('click', () => {
      ...
      audio.currentTime = 0;
      if (window.UniversalPlaybackHub) window.UniversalPlaybackHub.playAudio('recording', audio); else audio.play();
    });
    ...
    $('clearRecording')?.addEventListener('click', () => {
      window.UniversalPlaybackHub?.stop?.('recording');
      Recorder.clear();
    ...
    $('uploadBaseVoice')?.addEventListener('change', async ev => {
      ...
      if (!file) return;
      window.UniversalPlaybackHub?.stop?.('upload');
    ...
    $('previewUploadedVoice')?.addEventListener('click', () => {
      ...
      audio.currentTime = 0; if (window.UniversalPlaybackHub) window.UniversalPlaybackHub.playAudio('upload', audio); else audio.play();
    });
  ...
  const wire = () => {
    ...
    wireRecorder();
    if (window.UniversalPlaybackHub) {
      window.UniversalPlaybackHub.registerAudio('recording', $('recordedAudio'));
      window.UniversalPlaybackHub.registerAudio('upload', $('uploadedAudio'));
      document.querySelectorAll('[data-player-action]').forEach(btn => btn.addEventListener('click', async () => {
        ...
        if (action === 'play') {
          if (id === 'base') await window.UniversalPlaybackHub.playSpeech('base', $('sampleText').value || Synth.PREVIEW_PHRASE, state.profile, { npc: state.npc, accentCadence: state.biomeCadence, biomeAccent: state.biomeAccent, speakerReference: getSpeakerReference() });
          else if (id === 'emotion') { const { key, intensity } = getCurrentEmotionSelection(); await window.UniversalPlaybackHub.playSpeech('emotion', $('sampleText').value || Synth.PREVIEW_PHRASE, state.profile, { npc: state.npc, emotionKey:key, emotionIntensity:intensity, accentCadence: state.biomeCadence, biomeAccent: state.biomeAccent, speakerReference: getSpeakerReference() }); }
          else if (id === 'recording') window.UniversalPlaybackHub.playAudio('recording', $('recordedAudio'));
          else if (id === 'upload') window.UniversalPlaybackHub.playAudio('upload', $('uploadedAudio'));
        } else if (action === 'pause') window.UniversalPlaybackHub.pause(id);
        else if (action === 'resume') window.UniversalPlaybackHub.resume(id);
        else if (action === 'stop') window.UniversalPlaybackHub.stop(id);
        else if (action === 'rewind') window.UniversalPlaybackHub.rewind(id, 5);
        else if (action === 'forward') window.UniversalPlaybackHub.forward(id, 5);
      }));
    }
    ...
    $('previewSynth').addEventListener('click', async () => {
      ...
      await (window.UniversalPlaybackHub ? window.UniversalPlaybackHub.playSpeech('base', $('sampleText').value || Synth.PREVIEW_PHRASE, state.profile, { npc: state.npc, accentCadence: state.biomeCadence, biomeAccent: state.biomeAccent, speakerReference: getSpeakerReference() }) : Synth.speakBrowserPreview($('sampleText').value || Synth.PREVIEW_PHRASE, state.profile, { npc: state.npc, accentCadence: state.biomeCadence, biomeAccent: state.biomeAccent, speakerReference: getSpeakerReference() }));
      updateOutputs();
    });
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    await initTaxonomy();
    await loadManifest();
    await applyNpc({ name:'Universal NPC Voice', race:'Human', genderIdentity:'Non-Binary', class:'Bard', alignment:'True Neutral', biome:'none', personality:'warm, adaptive, inclusive storyteller', voiceInfluences: state.influences });
  });
})();
