...
(function(){
  ...
  function collectPanel(prefix){
    ...
    const payload={
      schema:'universal.dropdownBuilder.embeddedPagePayload.v1',
  ...
  function sheetApi(){ try { return sheetWin()?.UniversalSheetApi || null; } catch(e){ return null; } }
  ...
  function collectAllSiteData(){
    ...
    const data={
      schema:'universal.fullInteractiveCharacterSite.v2',
  ...
  function pushAllToSheet(){
    ...
    else sheetWin()?.postMessage({type:'universal-apply-site-payload', payload}, '*');
  ...
  function downloadSheetHtml(){
    ...
    const slug=(c?.identity?.name || 'universal-character-sheet').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'') || 'universal-character-sheet';
  ...
  function wireButtons(){
    ...
    q('downloadCharacterJsonBtn')?.addEventListener('click',()=>downloadBlob('universal-full-character-site.json', JSON.stringify(collectAllSiteData(), null, 2), 'application/json'));
  ...
  function init(){
    ...
    window.addEventListener('message', ev=>{ if(ev.data?.type==='universal-sheet-ready') pushAllToSheet(); });
  }
  window.UniversalFullSiteBridge={collectAllSiteData,pushAllToSheet,pullSheetToSite,downloadSheetHtml,collectPanel};
