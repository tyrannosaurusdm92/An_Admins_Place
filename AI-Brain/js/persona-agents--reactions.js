...
(function () {
  ...
  const LS = (window.LifeSimulation = window.LifeSimulation || {});
  ...
  function technologyContext(era) {
    const normalizedEra = LS.era?.clampEra(era) ?? Math.max(3, Math.min(10, Number(era) || 5));
    const profile = LS.era?.reactionTechnologyProfile(normalizedEra) || bundle.technology?.find?.(item => Number(item.order) === normalizedEra);
    return {
      profile,
  ...
  function personalize(reaction, npc, state, seed) {
    ...
    return {
      ...
      semanticReactionId: reaction.id,
      icon: iconPath(reaction),
      visualNamespace: reaction.namespace || "core",
      visualSource: "semantic-core-selection",
      visualConfidence: 1,
  ...
  function resolveVisual(personalized, npc, state, block) {
    if (!personalized) return personalized;
    const resolved = LS.homeworldContext?.resolve?.({ state, npc, block, semanticReactionId: personalized.semanticReactionId || personalized.reactionId, activityId: block?.homeworldActivityId, explicitVisualPath: block?.visualOverridePath, coreVisualPath: personalized.icon });
    return resolved ? { ...personalized, icon: resolved.visualPath, ...resolved } : personalized;
  }

  LS.reactions = Object.freeze({ get, list, categories, iconPath, legacyTarget, available, select, personalize, record, resolveVisual, technologyContext, coreCount: core.size, expansionCounts: Object.fromEntries([...expansions].map(([id, map]) => [id, map.size])) });
})();
