...
(function (global) {
  ...
  function resolve(raceId, lineageId, genderIdentity, state = LS.store.get()) {
    const builtIn = exact.get(`${raceId}|${lineageId || ""}|${genderIdentity}`)
      ...
      || (byRace.get(raceId) || [])[0];
    if (builtIn) return { ...builtIn, source: "external-library", available: null };
  ...
  function tokenMarkup(npc, options = {}) {
    ...
    const resolved = npc.token?.relativePath ? npc.token : resolve(npc.raceId, npc.lineageId, npc.genderIdentity, state);
    ...
    const portrait = resolved.relativePath
  ...
  function previewMarkup({ raceId, lineageId, genderIdentity, borderId, dataUrl, name = "Preview" }) {
    const mock = {
      ...
      simulation: { currentReaction: { label: "Creating a race", icon: "assets/reactions/core/icons/technology_operations/designing_a_system.svg", conversationChance: 1 } }
    };
