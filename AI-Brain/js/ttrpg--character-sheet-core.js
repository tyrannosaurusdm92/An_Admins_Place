...
function updateLiveSummary(){
  const name = getByKey('characterName')?.value || 'Universal Character'; qsa('[data-live="characterName"]').forEach(el=>el.textContent=name);
...
function collectSheet(){
  ...
  obj._meta={exportedAt:new Date().toISOString(), generatedBy:'Universal Profile D&D Sheet', schema:APP_OPTIONS.schema, sourceTitle:APP_OPTIONS.sourceTitle}; return obj;
}
function loadSheet(obj){
  ...
  updateCalculated(); refreshArtSlots(); refreshAudioPlayer(); if(window.refreshUniversalDropdownBuilder) window.refreshUniversalDropdownBuilder({fromLoad:true});
}
function exportJson(){const blob=new Blob([JSON.stringify(collectSheet(),null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=(getByKey('characterName')?.value||'universal-character').toLowerCase().replace(/[^a-z0-9]+/g,'-')+'.json'; a.click(); URL.revokeObjectURL(a.href);}
...
function clearSheet(){
  ...
  updateCalculated(); refreshArtSlots(); refreshAudioPlayer(); if(window.refreshUniversalDropdownBuilder) window.refreshUniversalDropdownBuilder({fromClear:true});
...
function setupFloatingNav(){
  ...
  const saved=localStorage.getItem('universal-floating-nav-pos');
  if(saved){
  ...
  handle.addEventListener('pointerup',e=>{
    ...
    localStorage.setItem('universal-floating-nav-pos',JSON.stringify({left:rect.left,top:rect.top}));
    if(!moved){
