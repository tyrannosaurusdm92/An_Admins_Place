...
(function(){
  ...
  U.normalizeFileName = (settlement, province='', ext='') => {
    const base = [settlement || 'UniversalSettlement', province || 'Province'].filter(Boolean).join('_');
  ...
  U.seededRand = seed => { let x=U.hashCode(seed||'Universal'); return () => { x ^= x << 13; x ^= x >>> 17; x ^= x << 5; return ((x>>>0) / 4294967296); }; };
