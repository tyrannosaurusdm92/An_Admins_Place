...
/* Universal Voice Studio - smoothed browser-safe vocal model */
(() => {
  'use strict';
  ...
  const buildModel = (profile = {}, opts = {}) => {
    ...
    const Vocoder = window.UniversalVocoderMath;
    const Prosody = window.UniversalProsodyMath;
    const EmotionCurves = window.UniversalEmotionCurveMath;
    const SpeakerRef = window.UniversalSpeakerReferenceMath;
    ...
    const humanizer = window.UniversalVoiceHumanization || null;
    ...
    if (window.UniversalNaturalnessGuard) basePitch = window.UniversalNaturalnessGuard.stablePitch(p, { pitch: basePitch });
  ...
  window.UniversalVocalModel = { buildModel, chooseVoice, scoreVoice, describeModel };
})();
