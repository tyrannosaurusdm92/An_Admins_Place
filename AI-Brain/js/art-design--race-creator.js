...
(function (global) {
  ...
  function saveRace() {
    ...
    LS.app.toast(`${saved.name} is now part of the integrated UniversalSimulator race registry.`);
    LS.app.renderAll();
  }
  ...
  function bind() {
    ...
    document.querySelector("#raceGenderIdentity").innerHTML = LS.CONFIG.genderIdentities.map(identity => `<option>${identity}</option>`).join("");
