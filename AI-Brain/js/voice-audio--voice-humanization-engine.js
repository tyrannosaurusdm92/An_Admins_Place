...
/* Universal Voice Studio - resource-informed humanization engine
   Uses derived feature metadata from the user's supplied media/JSON packs. It does not embed original audio. */
(() => {
  'use strict';
  const refs = () => window.UNIVERSAL_VOICE_HUMANIZATION_REFERENCES || {};
  const speechPack = () => window.UNIVERSAL_SPEECH_PATTERN_PACK || {};
  ...
  const describeReference = (npc={}, accent=null) => {
    ...
    return { resourceCounts:{...(r.resourceCounts||{}), referenceAudioLearning: window.UNIVERSAL_REFERENCE_AUDIO_LEARNING?.overall || null}, biomeKey:r.biomeKey, confidence:r.confidence, averagedReference:avg, closestVoiceExample: closest.npc ? { npc:closest.npc, biome:closest.biome, race:closest.race, class:closest.class, genderIdentity:closest.genderIdentity, features:closest.feature } : null };
  };

  window.UniversalVoiceHumanization = { normalizeBiome, referenceFor, controlsFor, cadenceFor, humanizeAccentText, profileNudgeFromSpeechPatterns, describeReference };
})();
