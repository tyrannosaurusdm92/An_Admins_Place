...
(function (global) {
  ...
  function applyPayload(payload, sourceName = "Imported JSON") {
    ...
    LS.store.update(state => {
      const isProject = payload?.schema === "lifesimulation.project.v5" || payload?.project?.projectId || payload?.schema?.startsWith?.("lifesimulation.project");
      if (isProject) {
  ...
  async function readFile(file) {
    ...
    if (extension === "zip") {
      ...
      if (!reports.length) throw new Error("No compatible LifeSimulation JSON, LIFESIM, or DOCX was found in this ZIP.");
