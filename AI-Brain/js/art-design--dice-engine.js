...
const DICE = (function() {
    ...
    var vars = { //todo: make these configurable on init
        ...
        material_options: {
            // Geometry-only base: the selected dice theme paints the generated face textures.
            // Keep the mesh material white so old generic/yellow/black shell tint cannot bleed through.
            specular: 0x8b7a57,
            color: 0xffffff,
            shininess: 55,
            shading: THREE.FlatShading,
        },
        label_color: '#24201a', //numbers on dice
        dice_color: '#efe6d2',
        ambient_light_color: 0xf8f1e4,
        ...
        // Emergency-safe fallback: never use the old yellow/ivory demo shell.
        // Normal rolls replace this immediately with the selected dropdown/cache style.
        default_theme: { slug: 'selected-style-missing', name: 'Selected style not synced yet', description: 'temporary cyan debug skin; parent selection should replace this before rolls', base: '#02131b', base2: '#06d9ff', ink: '#ffffff', edge: '#00ffff', glow: '#9ffcff' },
        current_theme: null,
        theme_pool: []
    ...
    function normalize_hex(value, fallback) {
        ...
        var match = text.match(/#[0-9a-fA-F]{3,8}/);
        return match ? match[0] : fallback;
    }

    function hex_to_number(value, fallback) {
        var hex = normalize_hex(value, fallback || '#efe6d2').replace('#', '');
        if (hex.length === 3) hex = hex.split('').map(function(ch) { return ch + ch; }).join('');
        return parseInt(hex.slice(0, 6), 16);
    }

    function hex_to_rgb(value, fallback) {
        var hex = normalize_hex(value, fallback || '#efe6d2').replace('#', '');
        if (hex.length === 3) hex = hex.split('').map(function(ch) { return ch + ch; }).join('');
    ...
    function rgba(value, alpha, fallback) {
        var c = hex_to_rgb(value, fallback || '#efe6d2');
        return 'rgba(' + c.r + ',' + c.g + ',' + c.b + ',' + alpha + ')';
    }

    function pick_theme() {
        var pool = Array.isArray(vars.theme_pool) ? vars.theme_pool.filter(function(theme) { return theme && typeof theme === 'object'; }) : [];
        return (pool.length ? pool[Math.floor(Math.random() * pool.length)] : vars.default_theme) || vars.default_theme;
    }

    function apply_theme(theme) {
        theme = (theme && typeof theme === 'object') ? theme : vars.default_theme;
        vars.current_theme = theme;
        vars.dice_color = normalize_hex(theme.base || theme.base2 || theme.edge, vars.default_theme.base);
        vars.label_color = normalize_hex(theme.ink || theme.glow, vars.default_theme.ink);
        // The outer shell is intentionally not pre-colored anymore.  The canvas face
        // textures below fill the geometry from the selected CSS-derived dice choice.
        // Using white here prevents the old generic yellow/black material tint.
        vars.material_options.color = 0xffffff;
        vars.material_options.specular = hex_to_number(theme.edge || theme.glow || vars.default_theme.edge, vars.default_theme.edge);
        vars.material_options.shininess = /metal|steel|silver|gold|brass|bronze|copper|glass|ice|opal|gem|jade|quartz|moonstone|sapphire|emerald|amethyst/i.test((theme.slug || '') + ' ' + (theme.description || '')) ? 82 : 55;
    }

    function draw_theme_face_background(context, canvas, back_color) {
        var theme = vars.current_theme || vars.default_theme;
        var base = normalize_hex(theme.base || back_color, vars.default_theme.base);
        var base2 = normalize_hex(theme.base2 || theme.glow || base, base);
        var edge = normalize_hex(theme.edge || theme.ink || base2, vars.default_theme.edge);
        var glow = normalize_hex(theme.glow || base2, '#ffffff');
        var slug = String((theme.slug || '') + ' ' + (theme.name || '') + ' ' + (theme.description || '')).toLowerCase();
        var w = canvas.width, h = canvas.height;
        var linear = context.createLinearGradient(0, 0, w, h);
        linear.addColorStop(0, rgba(glow, 0.33, '#ffffff'));
        linear.addColorStop(0.18, base2);
        linear.addColorStop(0.58, base);
        linear.addColorStop(1, rgba(edge, 0.92, vars.default_theme.edge));
        context.fillStyle = linear;
        context.fillRect(0, 0, w, h);

        var radial = context.createRadialGradient(w * 0.28, h * 0.18, 1, w * 0.42, h * 0.38, Math.max(w, h) * 0.86);
        radial.addColorStop(0, rgba('#ffffff', 0.42, '#ffffff'));
        radial.addColorStop(0.34, rgba(glow, 0.16, '#ffffff'));
        radial.addColorStop(1, rgba('#000000', 0.30, '#000000'));
        context.fillStyle = radial;
        context.fillRect(0, 0, w, h);

        context.save();
        context.globalCompositeOperation = 'source-over';
        context.lineWidth = Math.max(2, w * 0.018);
        context.strokeStyle = rgba(edge, 0.74, vars.default_theme.edge);
        for (var r = 0; r < 3; r++) context.strokeRect(r * w * 0.035, r * h * 0.035, w - r * w * 0.07, h - r * h * 0.07);

        if (/dragon|scale|mermaid|serpent/.test(slug)) {
            context.strokeStyle = rgba(edge, 0.45, vars.default_theme.edge);
            context.lineWidth = Math.max(1, w * 0.01);
            var scale = w / 5;
            for (var y = -scale; y < h + scale; y += scale * 0.62) {
                for (var x = -scale; x < w + scale; x += scale) {
                    context.beginPath();
                    context.arc(x + ((Math.round(y / scale) % 2) ? scale / 2 : 0), y, scale * 0.42, 0, Math.PI);
                    context.stroke();
                }
            }
        } else if (/galaxy|nebula|celestial|void|plasma|storm|aurora|lunar|solar|star/.test(slug)) {
            context.fillStyle = rgba(glow, 0.72, '#ffffff');
            for (var i = 0; i < 42; i++) {
                var x1 = (i * 37 % 100) / 100 * w;
                var y1 = (i * 53 % 100) / 100 * h;
                var rad = (i % 5 === 0 ? 2.4 : 1.2) * (w / 256);
                context.beginPath();
                context.arc(x1, y1, rad, 0, Math.PI * 2);
                context.fill();
            }
        } else if (/clockwork|steampunk|gear/.test(slug)) {
            context.strokeStyle = rgba(edge, 0.58, vars.default_theme.edge);
            context.lineWidth = Math.max(1.5, w * 0.012);
            for (var g = 0; g < 3; g++) {
                var cx = w * (0.25 + g * 0.25), cy = h * (0.28 + (g % 2) * 0.32), rr = w * (0.12 + g * 0.025);
                context.beginPath();
                context.arc(cx, cy, rr, 0, Math.PI * 2);
                context.stroke();
                for (var t = 0; t < 12; t++) {
                    var a = Math.PI * 2 * t / 12;
                    context.beginPath();
                    context.moveTo(cx + Math.cos(a) * rr * 0.7, cy + Math.sin(a) * rr * 0.7);
                    context.lineTo(cx + Math.cos(a) * rr * 1.18, cy + Math.sin(a) * rr * 1.18);
                    context.stroke();
                }
            }
        } else if (/glitter|rainbow|opal|flower|resin|candy|honey|liquid|swirl|acrylic|glass/.test(slug)) {
            for (var d = 0; d < 36; d++) {
                context.fillStyle = d % 2 ? rgba(glow, 0.36, '#ffffff') : rgba(edge, 0.28, vars.default_theme.edge);
                context.beginPath();
                context.arc((d * 29 % 100) / 100 * w, (d * 71 % 100) / 100 * h, (2 + (d % 4)) * (w / 256), 0, Math.PI * 2);
                context.fill();
            }
            context.strokeStyle = rgba('#ffffff', 0.18, '#ffffff');
            context.lineWidth = Math.max(1, w * 0.008);
            for (var s = -h; s < w; s += w * 0.17) {
                context.beginPath();
                context.moveTo(s, h);
                context.bezierCurveTo(s + w * 0.24, h * 0.68, s + w * 0.08, h * 0.22, s + w * 0.42, 0);
                context.stroke();
            }
        } else if (/bone|parchment|ancient|norse|draugr|paper|moss|forest|jade|stone|quartz/.test(slug)) {
            context.strokeStyle = rgba(edge, 0.28, vars.default_theme.edge);
            context.lineWidth = Math.max(1, w * 0.007);
            for (var l = 0; l < 18; l++) {
                context.beginPath();
                context.moveTo((l * 13 % 100) / 100 * w, 0);
                context.lineTo(((l * 13 + 34) % 100) / 100 * w, h);
                context.stroke();
            }
        } else if (/metal|steel|silver|gold|brass|bronze|copper|iron|paladin|queen|arcane/.test(slug)) {
            var metal = context.createLinearGradient(0, h, w, 0);
            metal.addColorStop(0, rgba('#000000', 0.18, '#000000'));
            metal.addColorStop(0.24, rgba('#ffffff', 0.22, '#ffffff'));
            metal.addColorStop(0.50, rgba(edge, 0.28, vars.default_theme.edge));
            metal.addColorStop(0.74, rgba('#ffffff', 0.16, '#ffffff'));
            metal.addColorStop(1, rgba('#000000', 0.22, '#000000'));
    ...
    that.set_theme_pool = function(pool) {
        vars.theme_pool = Array.isArray(pool) ? pool.filter(function(theme) { return theme && typeof theme === 'object'; }) : [];
        // Keep the active selected/cache CSS-derived skin as the engine default too.
        // This prevents the renderer from falling back to the old generic/yellow material
        // when a roll is triggered from inside the iframe or before a roll payload arrives.
        if (vars.theme_pool.length) {
            vars.default_theme = $t.copyto(vars.theme_pool[0], {});
            apply_theme(vars.default_theme);
        } else {
            apply_theme(vars.default_theme);
        }
    ...
    that.get_theme_pool = function() {
        return Array.isArray(vars.theme_pool) ? vars.theme_pool.slice() : [];
    }
    ...
    that.dice_box.prototype.create_dice = function(type, pos, velocity, angle, axis) {
        var theme = pick_theme();
        apply_theme(theme);
        reset_material_cache();
        var dice = threeD_dice['create_' + type]();
        dice.userData = Object.assign({}, dice.userData || {}, { diceStyleName: theme.name || theme.slug || 'selected CSS-derived style' });
    ...
    threeD_dice.create_d4 = function() {
        ...
        return new THREE.Mesh(this.d4_geometry, new THREE.MeshFaceMaterial(
    ...
    threeD_dice.create_d6 = function() {
        ...
        return new THREE.Mesh(this.d6_geometry, new THREE.MeshFaceMaterial(
    ...
    threeD_dice.create_d8 = function() {
        ...
        return new THREE.Mesh(this.d8_geometry, new THREE.MeshFaceMaterial(
    ...
    threeD_dice.create_d9 = function() {
        ...
        return new THREE.Mesh(this.d10_geometry, new THREE.MeshFaceMaterial(
    ...
    threeD_dice.create_d12 = function() {
        ...
        return new THREE.Mesh(this.d12_geometry, new THREE.MeshFaceMaterial(
                create_dice_materials(CONSTS.standart_d20_dice_face_labels, vars.scale / 2, 1.0)));
    }

    threeD_dice.create_d20 = function() {
        if (!this.d20_geometry) this.d20_geometry = create_d20_geometry(vars.scale);
        return new THREE.Mesh(this.d20_geometry, new THREE.MeshFaceMaterial(
    ...
    function create_dice_materials(face_labels, size, margin) {
        function create_text_texture(text, color, back_color) {
            ...
            draw_theme_face_background(context, canvas, back_color);
...
// Make the const-backed DICE API reachable from message-bridge code.
// Top-level const values do not automatically become window properties,
// so the previous build ignored selected style payloads and stayed on fallback colors.
