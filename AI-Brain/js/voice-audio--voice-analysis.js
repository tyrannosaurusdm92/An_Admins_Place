...
/* Universal Voice Studio - optional recording analysis using STFT/mel/LPC/F0 math */
(() => {
  'use strict';
  const Core = window.UniversalVoiceCore;
  const MathCore = () => window.UniversalVoiceMathCore;
  const STFT = () => window.UniversalSTFTMath;
  const Mel = () => window.UniversalMelMath;
  const LPC = () => window.UniversalLPCMath;
  const Speaker = () => window.UniversalSpeakerReferenceMath;
  const getCtx = () => window.__universalVoiceCtx || (window.__universalVoiceCtx = new (window.AudioContext || window.webkitAudioContext)());
  const loadManifest = async (url = 'data/voice-audio-manifest.json') => {
    if (Array.isArray(window.UNIVERSAL_VOICE_MANIFEST)) return window.UNIVERSAL_VOICE_MANIFEST.map(x => ({...x}));
  ...
  const analyzeClip = async (clip) => {
    ...
    if (window.UniversalSpeakerReferenceMath) liveAnalysis.speakerReference = window.UniversalSpeakerReferenceMath.buildReference({ liveAnalysis }, clip);
  ...
  const scoreClipForProfile = (clip, profile = {}, npc = {}) => {
    ...
    const f0Target = window.UniversalVoiceCore.profileToEngine(profile).f0;
  ...
  window.UniversalVoiceAnalysis = { getCtx, loadManifest, decodeClip, analyzeAudioBuffer, analyzeClip, metricsToProfile, scoreClipForProfile, selectClips };
})();
