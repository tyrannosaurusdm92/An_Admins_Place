...
(function (global) {
  ...
  function renderDashboard() {
    ...
    byId("projectMetrics").innerHTML = [
      ...
      metric(LS.species.allRaces(state).length, "Available races", "161 canonical + homebrew"), metric(LS.tokens.borders.length, "Token borders", "every SVG selectable")
    ].join("");
  ...
  function updateNpcRaceSelectors() {
    ...
    byId("npcRace").innerHTML = `<option value="">Any race${categoryId ? " in this category" : " across all 23 categories"}</option>${raceChoices.map(race => `<option value="${race.raceId}"${race.raceId === previousRace ? " selected" : ""}>${LS.util.escape(race.name)}${categoryId ? "" : ` · ${LS.util.escape(race.category)}`}</option>`).join("")}`;
  ...
  function updateLocationSelector() {
    const state = LS.store.get();
    const current = byId("npcLocation")?.value || "";
    if (byId("npcLocation")) byId("npcLocation").innerHTML = `<option value="">Any or none</option>${state.locations.map(location => `<option value="${location.locationId}"${location.locationId === current ? " selected" : ""}>${LS.util.escape(location.name)}</option>`).join("")}`;
  }
  function renderPeople() {
    const state = LS.store.get();
    updateLocationSelector();
    const query = byId("npcSearch").value.trim().toLowerCase();
    const records = state.npcs.filter(npc => !query || `${npc.name} ${npc.raceName} ${npc.lineageName || ""} ${npc.profession} ${npc.genderIdentity} ${npc.simulation?.currentReaction?.label || ""}`.toLowerCase().includes(query));
    ...
    byId("npcGrid").innerHTML = records.length ? records.map(npc => {
      ...
      return `<article class="record-card npc-card">
        <header><div class="token-cell">${LS.tokens.tokenMarkup(npc, { state })}</div><div class="record-title"><h4>${LS.util.escape(npc.name)}</h4><p class="sub">${LS.util.escape(npc.raceName)}${npc.lineageName ? ` · ${LS.util.escape(npc.lineageName)}` : ""} · ${LS.util.escape(npc.genderIdentity)}</p></div><span class="badge">${LS.util.escape(npc.simulation?.currentReaction?.label || "Available")}</span></header>
        <dl><dt>Profession</dt><dd>${LS.util.escape(npc.profession)}</dd><dt>Current place</dt><dd>${LS.util.escape(location?.name || "Unassigned")}</dd><dt>Goal</dt><dd>${LS.util.escape(npc.goals?.[0] || "User-defined")}</dd><dt>Pronouns</dt><dd>${LS.util.escape(npc.pronouns?.label || "they/them")}</dd></dl>
    ...
    }).join("") : empty("No NPCs match this search. Generate a batch with any of the 23 race categories.");
  }
  ...
  function renderLocations() {
    ...
    byId("locationGrid").innerHTML = records.length ? records.map(location => {
      ...
      return `<article class="record-card location-card"><header><div><h4>${LS.util.escape(location.name)}</h4><p class="sub">${LS.util.escape(location.type)} · ${LS.util.escape(location.biomePath || "No biome assigned")}</p></div><span class="badge">${present.length} PRESENT</span></header>
        <dl><dt>Services</dt><dd>${LS.util.escape((location.services || []).join(", ") || "None yet")}</dd><dt>Hours</dt><dd>${LS.util.escape(location.hours ? `${location.hours.open}–${location.hours.close}` : "User-defined")}</dd><dt>Access</dt><dd>${LS.util.escape(location.accessibility?.notes || (location.accessibility?.stepFree ? "Step-free route recorded" : "Needs review"))}</dd><dt>Hook</dt><dd>${LS.util.escape(location.plotHooks?.[0] || "None yet")}</dd></dl>
        ...
        <div class="card-actions"><button class="small ghost" data-edit-record="locations" data-record-id="${location.locationId}">Edit</button><button class="small danger" data-delete-record="locations" data-record-id="${location.locationId}">Delete</button></div></article>`;
    }).join("") : empty("No locations match this search. Create places under any protected biome path.");
  }
  ...
  function renderRaces() {
    ...
    byId("raceMetrics").innerHTML = [metric(23, "Race categories", "22 fantasy + Alien-Folk"), metric(161, "Canonical parent races", "130 fantasy + 31 alien"), metric(7664, "Ordered mixed heritages", "complete protected registry"), metric(manifest.expectedTokens, "Expected portrait files", "all names and paths embedded")].join("");
    renderRaceCatalog();
  ...
  function validate() {
    ...
    if (LS.species.categories.length !== 23) errors.push("Race category registry does not contain exactly 23 categories.");
    if (LS.species.builtInRaces.filter(race => race.categoryNumber <= 22).length !== 130) errors.push("Fantasy parent-race count is not 130.");
    if (LS.species.builtInRaces.filter(race => race.categoryNumber === 23).length !== 31) errors.push("Alien-Folk parent-race count is not 31.");
    if (global.LS_BOOTSTRAP.mixed.records.length !== 7664) errors.push("Ordered mixed-heritage registry is incomplete.");
    if (manifests.expectedTokens !== 3486) errors.push("External token filename index is incomplete.");
    if (manifests.borders !== 285 || !manifests.allBordersAssignable) errors.push("Not every token border is assignable.");
    if (LS.biomes.all.length !== 18 || LS.biomes.categories.length !== 5) errors.push("Canonical biome hierarchy is incomplete.");
    state.npcs.forEach(npc => {
      if (!npc.npcId || !npc.name) errors.push("An NPC is missing a stable ID or name.");
      if (!LS.CONFIG.genderIdentities.includes(npc.genderIdentity)) warnings.push(`${npc.name}: gender identity is user-defined and not in the 14 built-in options.`);
    ...
    const result = { lastRunAt: LS.util.now(), ready: errors.length === 0, errors: [...new Set(errors)], warnings: [...new Set(warnings)], counts: { categories: 23, fantasyRaces: 130, alienRaces: 31, mixedHeritage: 7664, expectedTokens: 3486, borders: 285, reactions: LS.reactions.coreCount, biomes: 18, npcs: state.npcs.length, locations: state.locations.length, factions: state.factions.length, quests: state.quests.length, dialogueThreads: Object.keys(state.conversations || {}).length, pendingDialogueReview: state.dialogueReview.length } };
  ...
  function renderTransfer() {
    ...
    byId("validationIssues").innerHTML = result.lastRunAt ? (issues.length ? issues.map(issue => `<article class="list-row issue ${issue.type}"><h4>${issue.type === "error" ? "Blocking" : "Review"}</h4><p>${LS.util.escape(issue.message)}</p></article>`).join("") : `<div class="notice success"><strong>Ready:</strong> registry counts and current records passed validation.</div>`) : `<p class="empty-state">Run validation to check registry completeness and current records.</p>`;
  }

  function renderAll() {
    renderDashboard(); renderPeople(); renderLocations(); renderRaces(); renderSimulation(); renderTransfer(); LS.conversations.render();
  }
  ...
  function bindProject() {
    const saveField = () => LS.store.update(state => { state.project.name = byId("projectName").value.trim() || "Untitled LifeSimulation Project"; state.project.genre = byId("projectGenre").value; state.project.description = byId("projectDescription").value; state.project.era = Number(byId("eraSlider").value); state.project.defaultBiomeId = byId("defaultBiome").value; return state; });
    ...
    byId("newProjectBtn").addEventListener("click", () => { if (confirm("Start a new LifeSimulation project? Export first if you need this project later.")) { LS.simulation.pause(); LS.store.reset(); setupSelectors(); renderAll(); toast("New project created."); } });
  }
  ...
  function bindGeneration() {
    ...
    byId("generateNpcsBtn").addEventListener("click", () => {
      const created = LS.simulation.generateNPCs({ count: byId("npcCount").value, seed: byId("npcSeed").value, categoryId: byId("npcCategory").value, raceId: byId("npcRace").value, lineageId: byId("npcLineage").value, genderIdentity: byId("npcGender").value, mixedHeritageId: selectedHeritageId(), era: byId("npcEra").value, locationId: byId("npcLocation").value, profession: byId("npcProfession").value.trim(), personality: byId("npcPersonality").value, conversationEnabled: byId("npcConversationEnabled").checked });
  ...
  function bindTransfer() {
    ...
    byId("exportPeopleBtn").addEventListener("click", () => LS.exporters.downloadNpcs(false)); byId("exportLocationsBtn").addEventListener("click", () => LS.exporters.downloadLocations(false)); byId("exportRacesBtn").addEventListener("click", LS.exporters.downloadCustomRaces);
  }
  ...
  function setupSelectors() {
    ...
    byId("npcGender").innerHTML = `<option value="">Any built-in identity</option>${LS.CONFIG.genderIdentities.map(identity => `<option>${identity}</option>`).join("")}`;
    byId("npcEra").innerHTML = optionsEra(true); byId("locationEra").innerHTML = optionsEra(true);
    byId("locationBiome").innerHTML = LS.biomes.groupedOptions("auto", true);
  ...
  function init() {
    ...
    setupSelectors(); bindProject(); bindGeneration(); bindSimulation(); bindTransfer(); bindGlobal(); LS.raceCreator.bind(); LS.conversations.bind();
