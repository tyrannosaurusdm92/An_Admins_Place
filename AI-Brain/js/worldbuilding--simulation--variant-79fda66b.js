...
(function (global) {
  ...
  function pronounsFor(identity, random) {
    const maps = {
      "Cis-Female": ["she", "her", "her", "herself", "she/her"], "Trans-Female": ["she", "her", "her", "herself", "she/her"],
      "Cis-Male": ["he", "him", "his", "himself", "he/him"], "Trans-Male": ["he", "him", "his", "himself", "he/him"]
    };
    const value = maps[identity] || (random() < 0.16 ? ["xe", "xem", "xyr", "xemself", "xe/xem"] : ["they", "them", "their", "themself", "they/them"]);
    return { subject: value[0], object: value[1], possessive: value[2], reflexive: value[3], label: value[4] };
  }
  ...
  function createNpc(options = {}, index = 0, state = LS.store.get()) {
    ...
    const genderIdentity = options.genderIdentity || LS.util.pick(LS.CONFIG.genderIdentities, random);
    ...
    const profession = options.profession || LS.util.pick(professionPool, random) || "community member";
    ...
    const npc = {
      ...
      aliases: [], genderIdentity, pronouns: pronounsFor(genderIdentity, random), bodyPlan: bodyPlanFor(race),
      ...
      cultureId: null, factionIds: [], profession, abilities: LS.util.sample(abilities, 3, random),
      ...
      public: { description: `${race.name}${lineage ? ` (${lineage.name})` : ""} ${profession}.`, knownFacts: [`Works as ${profession}.`], rumors: [], approved: true },
