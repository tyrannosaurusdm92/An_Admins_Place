...
function hashSeed(str) { let h=2166136261; for(const ch of String(str||'Universal')) { h ^= ch.charCodeAt(0); h = Math.imul(h,16777619); } return h>>>0; }
...
function makeCharacter() {
  ...
  const character = {
    schema:'universal.characterVoiceMerged.v1',
    ...
    stats,
    alignment,
    notes:{raceSummary:races.map(r=>({name:r.name, category:r.category, traits:r.canonTraits, abilities:r.universalAbilities, tendency:r.typicalPlayTendency})), classSummary:classSummary(cls)}
  };
...
function renderRaceNotes(c) {
  ...
  $('raceNotes').innerHTML = races.map(r=>`<article class="mini-card"><h3>${escapeHtml(r.name)}</h3><p><strong>Category:</strong> ${escapeHtml(r.category)}<br><strong>Creator God:</strong> ${escapeHtml(r.creatorGod)}<br><strong>Tendency:</strong> ${escapeHtml(r.typicalPlayTendency||'')}</p><p>${escapeHtml(r.description||'')}</p><p><strong>Traits:</strong> ${escapeHtml(r.canonTraits||r.sheet?.traitsText||'')}</p><p><strong>Universal abilities:</strong> ${escapeHtml(r.universalAbilities||'')}</p></article>`).join('') || '<p class="muted">Choose a race to see notes.</p>';
}
...
function getBiomeAccentForVoice(profile={}, npc=buildVoiceNpcObject()) {
  if (window.UniversalBiomeAccents?.applyAccent) {
    try {
      const applied = window.UniversalBiomeAccents.applyAccent(profile, npc, { biomeKey:npc.biome, influence:100 });
...
function shapeVoicePreviewText(text='', npc=buildVoiceNpcObject(), accent=null, traits={}) {
  ...
  if (!construct && window.UniversalVoiceHumanization?.humanizeAccentText) {
    try { phrase = window.UniversalVoiceHumanization.humanizeAccentText(phrase, npc, accent) || phrase; } catch {}
  }
...
function makeVoiceExport(source='manual') {
  ...
  const profile={
    schema:'universal.voiceProfile.v1', source, generatedAt:new Date().toISOString(),
    ...
    speechPreviewEngine: window.UniversalSpeechApi ? 'UniversalSpeechApi + naturalness guard' : 'Browser SpeechSynthesis safe fallback',
...
async function speakPreview() {
  const v=makeVoiceExport(lastVoice?.source || 'manual-preview'); renderVoice(v);
  if(!('speechSynthesis' in window)) { $('voiceStatus').textContent='This browser does not expose speech synthesis.'; return; }
  const npc = buildVoiceNpcObject();
  const accentBundle = getBiomeAccentForVoice(v.sliders, npc);
  const profile = naturalizeVoiceTraits(accentBundle.accentedProfile || v.sliders, npc.race);
  const phrase = shapeVoicePreviewText($('previewText').value || 'Voice preview ready.', npc, accentBundle.biomeAccent, profile);
  const opts = { profile, npc, biomeAccent:accentBundle.biomeAccent, accentCadence:accentBundle.accentCadence, accentInfluence:100 };
  try {
    if (window.UniversalSpeechApi?.say) {
      const result = await window.UniversalSpeechApi.say(phrase, v.speechApiPreview, opts);
      $('voiceStatus').textContent = `${v.source} profile ready • ${v.descriptors.pitch} pitch • ${v.descriptors.speed} speed • ${v.descriptors.roughness} texture • ${accentBundle.biomeAccent?.name || 'no biome accent'} • ${result.voice || 'browser default'} voice`;
      return;
    }
  } catch(err) { console.warn('Advanced speech preview failed; using safe fallback.', err); }
  window.speechSynthesis.cancel();
  const u=new SpeechSynthesisUtterance(phrase);
  u.rate=v.speechApiPreview.rate; u.pitch=v.speechApiPreview.pitch; u.volume=v.speechApiPreview.volume;
...
function safeName(s,fallback='universal-export') { return (s||fallback).toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,60)||fallback; }
...
function wireEvents() {
  ...
  $('downloadCharacterJsonBtn').addEventListener('click',()=>{ const c=makeCharacter(); downloadText(safeName(c.identity.name,'universal-character')+'.json', JSON.stringify(c,null,2)); });
  ...
  $('speakPreviewBtn').addEventListener('click',speakPreview); $('stopSpeakBtn').addEventListener('click',()=>window.speechSynthesis?.cancel());
  $('copyVoiceJsonBtn').addEventListener('click',()=>copyText($('voiceExport').value));
  $('downloadVoiceJsonBtn').addEventListener('click',()=>downloadText(safeName($('voiceName').value,'universal-voice')+'.json',$('voiceExport').value));
  $('downloadVoiceJsBtn').addEventListener('click',()=>{ const v=makeVoiceExport(lastVoice?.source||'manual'); const js='window.UNIVERSAL_EXPORTED_VOICE_PROFILE = '+JSON.stringify(v,null,2)+';\n'; downloadText(safeName(v.name,'universal-voice')+'.js',js,'text/javascript'); });
