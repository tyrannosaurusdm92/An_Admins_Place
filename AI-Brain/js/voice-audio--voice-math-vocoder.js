...
/* Universal Voice Studio - transparent vocoder parameter set
   Classical WORLD-like parameters: F0, spectral envelope, and aperiodicity. */
(() => {
  'use strict';
  const M = () => window.UniversalVoiceMathCore;
  const Source = () => window.UniversalSourceFilterMath;
  const Formants = () => window.UniversalFormantMath;
  const Inst = () => window.UniversalInstabilityMath;
  const PSOLA = () => window.UniversalPSOLAMath;
  ...
  window.UniversalVocoderMath = { buildParams };
})();
