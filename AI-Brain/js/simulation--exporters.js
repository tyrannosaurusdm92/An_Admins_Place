...
(function(){
  ...
  E.exportZIP = async function(state){
    ...
    const blob=await zip.generateAsync({type:'blob'}); U.downloadBlob(blob, `${U.slug(state.settings?.settlementName || state.settings?.province || 'Universal') || 'Universal'}.zip`);
  };
  async function fetchText(path){ try{return await fetch(path).then(r=>r.ok?r.text():'');}catch{return '';} }
  function exportBase(state){ return U.normalizeFileName(state.settings?.settlementName || 'UniversalSettlement', state.settings?.province || 'Province', '').replace(/\.$/,'') || 'UniversalSettlement'; }
  E.buildSettlementHTML = function(state){
    ...
    const npcs=(state.npcs||[]).slice(0,500).map(n=>`<div class="card"><b>${U.escapeHTML(n.name)}</b><br>${U.escapeHTML(n.race)} · ${U.escapeHTML(n.genderIdentity)} · ${U.escapeHTML(n.pronouns)}<br>${U.escapeHTML(n.employment)}<br>${U.escapeHTML(n.alignment?.summary||'')}</div>`).join('');
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${U.escapeHTML(s.settlementName||'Universal Export')}</title><style>${css}</style><script type="application/json" data-universal-state>${escapeScript(JSON.stringify(state))}</script></head><body><h1>${U.escapeHTML(s.settlementName||s.province||'Universal Export')}</h1><p>${U.escapeHTML(s.province||'')} · ${U.escapeHTML(s.settlementType||'')} · ${U.escapeHTML(s.biome||'')}</p><section><h2>Summary</h2><p>Locations: ${state.locations?.length||0}. NPCs: ${state.npcs?.length||0}. Relationships: ${state.relationships?.length||0}. Travel records: ${state.travel?.length||0}.</p>${(s.tags||[]).map(t=>`<span class="tag">${U.escapeHTML(t)}</span>`).join('')}</section><section><h2>Locations Preview</h2><div class="grid">${locs}</div></section><section><h2>NPC Preview</h2><div class="grid">${npcs}</div></section><section><h2>Restore Data</h2><p>The complete generated state is embedded as application/json in this HTML and can be imported back into the generator.</p></section></body></html>`;
  };
  function escapeScript(s){ return s.replace(/<\//g,'<\\/'); }
  E.buildDocxBlob = async function(state){
    const zip=new JSZip(); const s=state.settings||{};
    const documentXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>${p('Universal Life Generator Export')}${p((s.settlementName||s.province||'Generated World')+' · '+(s.settlementType||'')+' · '+(s.biome||''))}${p('Locations: '+(state.locations?.length||0)+' | NPCs: '+(state.npcs?.length||0)+' | Relationships: '+(state.relationships?.length||0)+' | Travel Records: '+(state.travel?.length||0))}${p('Settings: '+JSON.stringify(s))}${p('Top Locations')}${(state.locations||[]).slice(0,80).map(l=>p(l.name+' — '+l.category+' / '+l.subcategory+' — '+l.description)).join('')}${p('Top NPCs')}${(state.npcs||[]).slice(0,120).map(n=>p(n.name+' — '+n.race+' — '+n.genderIdentity+' — '+(n.alignment?.summary||'')+' — '+n.employment)).join('')}<w:sectPr><w:pgSz w:w="12240" w:h="15840"/><w:pgMar w:top="1440" w:right="1440" w:bottom="1440" w:left="1440"/></w:sectPr></w:body></w:document>`;
  ...
  E.readme = state => `Universal Life Generator Export\n\nThis ZIP preserves generated NPC records, locations, relationships, schedules, GeoJSON pins, travel histories, and restore data.\n\nOpen html/${exportBase(state)}.html for a readable settlement export. Import data/full_restore_state.json back into life_generator.html to restore all records without regeneration.\n`;
  window.UniversalExporters = E;
})();
