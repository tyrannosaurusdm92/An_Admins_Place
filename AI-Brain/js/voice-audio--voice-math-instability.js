...
/* Universal Voice Studio - jitter, shimmer, tremor, and controlled human micro-variation */
(() => {
  'use strict';
  const M = () => window.UniversalVoiceMathCore;
  const norm = v => window.UniversalVoiceCore ? window.UniversalVoiceCore.normalizeTrait(v) : M().clamp(+v||5,0,10);
  const buildInstability = (profile={}, opts={}) => {
    ...
    return { jitter:M().round(jitter,4), shimmer:M().round(shimmer,4), tremorRateHz, tremorDepthSemitones, humanizationSeed: opts.seed || opts.name || 'UniversalVoice' };
  };
  const f0At = (baseF0, t, inst={}, contourSemitones=0) => {
    const seed=inst.humanizationSeed || 'UniversalVoice'; const jitterSemi=(M().seededSigned(seed, Math.floor(t*97))*12*(inst.jitter||0));
  ...
  const amplitudeAt = (baseGain, t, inst={}) => {
    const seed=inst.humanizationSeed || 'UniversalVoice'; const shimmer=(M().seededSigned(seed, Math.floor(t*131))* (inst.shimmer||0));
    return M().clamp(baseGain * (1 + shimmer), 0, 2);
  };
  window.UniversalInstabilityMath = { buildInstability, f0At, amplitudeAt };
})();
