...
/* Universal Voice Studio - Natural speech API bridge
...
(() => {
  ...
  const smoothControls = (controls={}, profile={}, opts={}) => {
    const human = window.UniversalVoiceHumanization || null;
    ...
    if (window.UniversalNaturalnessGuard) out = window.UniversalNaturalnessGuard.controlsFor(profile || {}, out, opts);
    return out;
  };
  ...
  window.UniversalSpeechApi = { isAvailable, loadVoices, chooseVoice, scoreVoice, say, stop, pause, resume, smoothControls };
