...
/* Universal Voice Studio - Inclusive race, class, alignment, identity, and full JSON-pack preset logic */
(() => {
  'use strict';
  ...
  const loadTaxonomy = async (url='data/voice-taxonomy.json') => {
    ...
    if (window.UNIVERSAL_VOICE_GENERATOR_PACK?.taxonomy) { taxonomyCache = window.UNIVERSAL_VOICE_GENERATOR_PACK.taxonomy; return taxonomyCache; }
    if (window.UNIVERSAL_VOICE_TAXONOMY) { taxonomyCache = window.UNIVERSAL_VOICE_TAXONOMY; return taxonomyCache; }
  ...
  const parseUniversalAxes = (npc = {}) => {
    const axes = npc.universalAlignment || npc.alignmentAxes || npc.axes || {};
  ...
  const buildPreset = async (npc = {}, influences = {}) => {
    ...
    return {
      ...
      dataSource: {
        pack: t.sourcePack || 'Universal Voice Generator JSON Pack',
  ...
  window.UniversalVoicePresets = { loadTaxonomy, genderAdjustments, classAdjustments, traditionalAlignmentAdjustments, categoryAdjustments, applyAdjustments, buildPreset, getRaceDefaults, getClassDefaults, personalityAdjustments, parseUniversalAxes };
})();
