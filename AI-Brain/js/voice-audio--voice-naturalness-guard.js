...
/* Universal Voice Studio - browser TTS naturalness guard
   Keeps deep/masculine and high/bright voices out of robotic pitch extremes. */
(() => {
  'use strict';
  const clamp=(n,min,max)=>Math.max(min,Math.min(max,Number.isFinite(+n)?+n:min));
  const extra=()=>window.UNIVERSAL_REFERENCE_AUDIO_LEARNING||{};
  const norm=p=>({
  ...
  window.UniversalNaturalnessGuard = { controlsFor, cadenceFor, stablePitch, describe };
})();
