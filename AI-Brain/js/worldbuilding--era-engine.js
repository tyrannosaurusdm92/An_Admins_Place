...
(function () {
  ...
  function clampEra(value) {
    const parsed = Number(value);
    return Math.max(LS.CONFIG.eraMin, Math.min(LS.CONFIG.eraMax, Math.round(Number.isFinite(parsed) ? parsed : 5)));
  }
  function getProfile(value) { return PROFILES.find(profile => profile.order === clampEra(value)) || PROFILES[0]; }
  ...
  function reactionTechnologyProfile(value) {
    ...
    return profiles.find(profile => Number(profile.order) === clampEra(value)) || null;
  }
  ...
  LS.era = Object.freeze({ min: 3, max: 10, profiles: PROFILES, clampEra, getProfile, recordEra, reactionTechnologyProfile, occupationPool, locationPool, vocabulary, compatibility });
})();
