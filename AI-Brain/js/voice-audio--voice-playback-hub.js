...
/* Universal Voice Studio - one-at-a-time media/speech playback hub */
(() => {
  'use strict';
  ...
  const stopAll = (exceptId=null) => {
    try { window.UniversalVoiceProsody?.stop?.(); } catch {}
    try { window.UniversalSpeechApi?.stop?.(); } catch {}
  ...
  const playSpeech = async (id, text, profile, opts={}) => {
    ...
    let result;
    try { result = await window.UniversalVoiceSynthesis.speakBrowserPreview(partial, profile, opts); }
  ...
  const pause = (id=activeId) => {
    ...
    try { window.UniversalSpeechApi?.pause?.(); window.speechSynthesis?.pause?.(); setStatus(id,'Paused'); } catch {}
  };
  const resume = (id=activeId) => {
    ...
    try { window.UniversalSpeechApi?.resume?.(); window.speechSynthesis?.resume?.(); setStatus(id,'Playing typed text'); } catch {}
  };
  const stop = (id=activeId) => {
    if (!id) { stopAll(null); return; }
    const audio = audios.get(id);
    if (audio) { try { audio.pause(); audio.currentTime = 0; } catch {} }
    try { window.UniversalVoiceProsody?.stop?.(); window.UniversalSpeechApi?.stop?.(); window.speechSynthesis?.cancel?.(); } catch {}
  ...
  window.UniversalPlaybackHub = { registerAudio, playAudio, playSpeech, pause, resume, stop, seek, rewind, forward, stopAll, estimateSeconds, textFromOffset };
})();
