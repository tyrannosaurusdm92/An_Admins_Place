/* Genericized for AI-Brain capability use. Provenance group: world-life-simulation-b. */
(() => {
  'use strict';
  ...
  const MAP_SCHEMA = 'universal.stationary-map.v1';
  ...
  const SYMBOLS = [
    ['castle', 'Castle', '♜'], ['city', 'City', '▦'], ['village', 'Village', '⌂'],
    ['tower', 'Tower', '♖'], ['ruins', 'Ruins', '⌑'], ['cave', 'Cave', '◒'],
    ['camp', 'Camp', '△'], ['port', 'Port', '⚓'], ['bridge', 'Bridge', '═'],
    ['ship', 'Ship', '◁'], ['tree', 'Tree', '♠'], ['mountain', 'Mountain', '▲'],
    ['star', 'Star', '✦'], ['danger', 'Danger', '⚠'], ['portal', 'Portal', '◎'],
    ['temple', 'Temple', '⌂'], ['mine', 'Mine', '⛏'], ['pin', 'Map Pin', '●']
  ];
  ...
  function runGenerator(kind) {
    ...
    if (kind === 'dungeon') generateDungeon(layer.ctx, rng, count, complexity);
  ...
  function generateDungeon(ctx, rng, count, complexity) {
  ...
  function serializeMapProject() {
    return {
      ...
      sources: [
        ...
        'Dungeon Map Generator by Ben Damer (MIT) — room and corridor generation concepts',
  ...
  function exportMapProject() {
    ...
    downloadBlob(new Blob([JSON.stringify(project, null, 2)], {type: 'application/json'}), `${safeName(project.title)}.belamap.json`);
  ...
  async function importMapProject(file) {
    ...
    if (project.schema !== MAP_SCHEMA || !Array.isArray(project.layers)) { toast('This is not a supported Universal stationary map project.', 'error'); return; }
    ...
    $('#mapProjectTitle').value = project.title || file.name.replace(/\.belamap\.json$/i, '');
    $('#mapType').value = project.mapType || 'world'; $('#mapScale').value = project.mapScale || 'none';
  ...
  async function saveMapBackend() {
    ...
    const payload = {
      action: 'saveUniversalStationaryMapProject',
      studio: CONFIG.id || 'Universal_3D_Visual_Studio',
      title: $('#mapProjectTitle').value || 'Untitled Map',
      mapType: $('#mapType').value,
      mapScale: $('#mapScale').value,
      width: state.width,
  ...
  function bindUI() {
    ...
    drop.addEventListener('drop', event => {
      ...
      if (!file) return;
      if (/\.json$/i.test(file.name)) importMapProject(file); else if (file.type.startsWith('image/')) importMapImage(file); else toast('Drop a map image or .belamap.json project.', 'error');
    });
