...
(function(){
  ...
  async function parseHtmlFile(file){
    ...
    const jsonBlocks = Array.from(doc.querySelectorAll('script[type="application/json"], script[data-universal-state]')).map(s=>s.textContent.trim()).filter(Boolean);
    const parsedBlocks=[];
  ...
  window.UniversalImporters = Importers;
})();
