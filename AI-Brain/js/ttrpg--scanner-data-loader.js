...
(() => {
  'use strict';
  const S = window.UniversalScanner;
  ...
  async function readBundledJson(name){
    if(window.UNIVERSAL_SCANNER_JSON_DATA && window.UNIVERSAL_SCANNER_JSON_DATA[name]) return window.UNIVERSAL_SCANNER_JSON_DATA[name];
  ...
  S.loadBundledResources = async function(){
    ...
    try{
      ...
      normalizeBundle({biome,race,class:cls,gender,emotion,math}, window.UNIVERSAL_SCANNER_JSON_DATA ? 'embedded JS data + json/ folder' : 'json/ folder');
