...
(function (global) {
  ...
  const BRIDGE_SCHEMA = 'worldbuilder.lifesimulation.galaxy.bridge.v1';
  ...
  function syncToLife(reason) {
    ...
    if (!g || !LS?.store) return { ok: false, reason: 'LifeSimulation is not ready.' };
    ...
    LS.store.mutate(reason || 'worldbuilder-galaxy-sync', state => {
      ...
      state.bridge.provenance.unshift({ provenanceId: uid('prov'), at: now(), source: 'WorldBuilder Setting-Agnostic Complete Edition', reason: reason || 'sync', manifestSchema: BRIDGE_SCHEMA });
  ...
  function addSpeciesDraft(draft) {
    const LS = global.LifeSimulation; if (!LS?.store) return { ok: false, reason: 'LifeSimulation is not ready.' };
  ...
  function bind() {
    document.addEventListener('worldbuilder:species-draft', event => {
      ...
      if (toast && result.ok) { toast.textContent = `${result.species.name} was saved as a LifeSimulation species candidate.`; toast.classList.add('show'); setTimeout(() => toast.classList.remove('show'), 3000); }
    });
