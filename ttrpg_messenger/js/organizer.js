...
const WORKSPACE_VIEWS={dashboard:'Organizer',tasks:'Tasks',calendar:'Calendar',availability:'Availability',library:'TTRPG System',admin:'Approvals'};
...
const Workspace={
  ...
  async open(view){
    ...
    await loadCampaignData();this.render();
  },
  ...
  render(){
    ...
    const visibleViews=['dashboard','tasks','calendar','availability','library',...(isRunner()||canManageOrganizer()||canManageLibrary()?['admin']:[])];
  ...
  renderView(view){if(CampaignData.loading)return '<div class="empty-workspace">Loading campaign organizer…</div>';if(view==='dashboard')return renderDashboard();if(view==='tasks')return renderTaskBoard();if(view==='calendar')return renderCalendar();if(view==='availability')return renderAvailability();if(view==='library')return renderRulesLibrary();if(view==='admin')return renderApprovals();return renderDashboard()},
  bind(shell){
    shell.onclick=e=>{
