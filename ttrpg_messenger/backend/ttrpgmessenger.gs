 ...
 * Configured web app: https://script.google.com/macros/s/AKfycbxxcMVPpAaa8bbVguBVwdRu7B1QkvGxgf5Gq2D-lLQUFoaAsBM3hhS6IDBsK9bdDbmdlA/exec
 * Apps Script library: 18ET55A9uVNx3IUzoAM_eRj8v7jqagPgjVdxil3P1SoUqrFnnAJp6CjVr (version 3)
 *
 * MERGED FEATURE SET
...
function routeRollDice_(ctx){
  var channel=requireChannel_(ctx.params.channelId,ctx.user.id);requirePermission_(channel.serverId,ctx.user.id,PERMISSIONS.ROLL_DICE);var parsed=parseDiceExpression_(ctx.params.expression||ctx.params.dice),rolled=rollDice_(parsed),personaId=String(ctx.params.personaId||'');if(personaId){var p=requirePersona_(personaId,ctx.user.id);if(p.serverId!==channel.serverId)throw new ApiError_('INVALID_PERSONA','Persona belongs to another server.');}
  var label=nullableText_(ctx.params.label,200),now=nowIso_(),roll=insert_('DiceRolls',{id:id_('rolld'),serverId:channel.serverId,channelId:channel.id,userId:ctx.user.id,personaId:personaId,expression:parsed.expression,label:label,total:rolled.total,detailJson:JSON.stringify(rolled.detail),messageId:'',createdAt:now});
  ...
  audit_(channel.serverId,ctx.user.id,'DICE_ROLLED','DICE_ROLL',roll.id,{expression:parsed.expression,total:rolled.total});return {id:roll.id,serverId:roll.serverId,channelId:roll.channelId,userId:roll.userId,personaId:personaId,expression:parsed.expression,label:label,total:rolled.total,detail:rolled.detail,message:message,createdAt:now};
}
