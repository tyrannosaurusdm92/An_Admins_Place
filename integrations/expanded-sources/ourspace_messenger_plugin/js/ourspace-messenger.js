(function (global) {
  ...
  function createDefaultState(config) {
    ...
    return {
      ...
      channels: [
        { id: config.defaultChannel || 'home', title: 'Home', icon: '💬', kind: 'channel', createdAt: now, description: 'Shared messenger space' },
        { id: 'media', title: 'Media', icon: '🖼️', kind: 'channel', createdAt: now, description: 'Images, GIFs, files, and clips' },
        { id: 'calls', title: 'Calls', icon: '📞', kind: 'channel', createdAt: now, description: 'Call invites and WebRTC signals' }
      ],
      messages: [
        {
          id: uid('msg'),
          ...
          authorId: 'system',
          kind: 'system',
          text: 'OurSpace Messenger is ready. Channels, attachments, calls, transcription, voice effects, stickers, backgrounds, and face filters are available in one plug-in.',
          createdAt: now,
          attachments: [],
          links: []
        }
  ...
  class MessengerApp {
    constructor(config) {
      ...
      this.storage.channels = this._dedupeById(this.storage.channels || [], createDefaultState(this.config).channels);
      this.storage.messages = this.storage.messages || [];
      this.storage.stickers = this.storage.stickers || [];
    ...
    save() {
      const clean = Object.assign({}, this.storage, { pendingAttachments: [] });
      writeStorage(clean);
    }

    get me() {
      return this.config.users.find(u => u.id === this.storage.currentUserId) || this.config.users[0] || { id: 'me', label: 'Me', avatar: '🙂' };
    }
    ...
    renderChannels() {
      ...
      const users = this.config.users.map(user => `<button class="os-user-row" data-user-id="${escapeHtml(user.id)}">
        <span class="os-avatar">${escapeHtml(user.avatar || '🙂')}</span>
        <span><strong>${escapeHtml(user.label || user.id)}</strong><br><small class="os-channel-meta">${user.system ? 'Alert/system contact' : 'Direct message'}</small></span>
      </button>`).join('');
      return `<aside class="os-channels ${this.openPanels.channels ? 'is-open' : ''}">
        <header class="os-pane-head">
          <div class="os-title-row">
            <div><h1 class="os-app-title">${escapeHtml(this.config.appName)}</h1><p class="os-subtitle">Unified chat, calls, media, voice, filters.</p></div>
        ...
        <div class="os-section-label">People & alerts</div>
        ...
        <div class="os-studio-body">
          <section class="os-card">
            ...
            <select class="os-select" id="os-current-user">
              ${this.config.users.filter(u => !u.system).map(user => `<option value="${escapeHtml(user.id)}" ${user.id === this.storage.currentUserId ? 'selected' : ''}>${escapeHtml(user.avatar || '')} ${escapeHtml(user.label || user.id)}</option>`).join('')}
            </select>
          </section>
          <section class="os-card">
            <h3>Voice changer</h3>
    ...
    bindEvents() {
      this.root.addEventListener('click', (event) => {
        const target = event.target.closest('[data-action], [data-channel-id], [data-user-id]');
        if (!target) return;
        if (target.dataset.channelId) return this.selectChannel(target.dataset.channelId);
        if (target.dataset.userId) return this.openDirectMessage(target.dataset.userId);
    ...
    openDirectMessage(userId) {
      ...
      if (!this.storage.channels.some(c => c.id === id)) {
        this.storage.channels.push({ id, title: user?.label || userId, icon: user?.avatar || '🙂', kind: 'dm', createdAt: new Date().toISOString(), description: user?.system ? 'System alert thread' : 'Direct message' });
      }
    ...
    async startCall(withVideo) {
      try {
        ...
        await this.sendCallSignal({ type: 'offer', description: offer });
        await this.addAndSendMessage({ kind: 'system', authorId: 'system', text: `${withVideo ? 'Video' : 'Audio'} call started. Waiting for answer.`, signal: { type: 'offer', description: offer }, attachments: [], links: [] });
    ...
    async sendCallSignal(signal) {
      const payload = { id: uid('signal'), channelId: this.storage.activeChannelId, authorId: this.storage.currentUserId, createdAt: new Date().toISOString(), signal };
    ...
    async pollBackend() {
      try {
        const result = await this.bridge.listMessages({ channelId: this.storage.activeChannelId, since: this.storage.lastSyncAt });
    ...
    normalizeIncomingMessages(result) {
      ...
      return candidates.map(item => Object.assign({
        ...
        authorId: item.authorId || item.userId || 'remote',
